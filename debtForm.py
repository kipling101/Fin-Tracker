...
def openDebtForm(userID):
        ...
        try:
            if userID and addDebtAmount and addDebtName and addDebtDate and addDebtAPR:
                ...
                cursor.execute("INSERT INTO payments (userID, paymentAmount, paymentDate, paymentAPR, paymentAddDate) VALUES (%s, %s, %s, %s, %s)", 
                               (userID, addDebtAmount, datetime.now(), addDebtAPR, datetime.now()))
    ...
    menu.createMenu(main, userID); print("HI", curDebt)
    
    if len(curDebt) != 0:
        figSize = 5.5, 4; print("DS")
    ...
    helpObject.bind_widget(enterDebtDate,balloonmsg="Enter the date the debt was taken out. Format: YYYY-MM-DD HH:MM:SS")
